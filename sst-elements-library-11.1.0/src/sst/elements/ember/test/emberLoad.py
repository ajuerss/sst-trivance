...
try:
    opts, args = getopt.getopt(sys.argv[1:], "", ["topo=", "shape=","hostsPerRtr=",
                 ...
                 "rtrArb=","embermotifLog=","rankmapper=", "motifAPI=", "boardShape=",
                 "bgPercentage=","bgMean=","bgStddev=","bgMsgSize=","netInspect=",
                 "detailedModelName=","detailedModelParams=","detailedModelNodes=",
                 "useSimpleMemoryModel","param=","paramDir=","statsModule=","statsFile=", "globalShape=", "fatTreeShape=", "algorithm=", "programName="])
...
for o, a in opts:
    if o in ("--shape"):
        ...
        routing_algo = a
    elif o in ("--boardShape"):
        boardShape = a
    elif o in ("--globalShape"):
        globShape = a
    elif o in ("--fatTreeShape"):
        fatTreeShape = a
...
if "" == netShape and "" == boardShape:
    if (boardShape == "" and netTopo == "hx"):
        print("Error, --boardShape must be specified!")
        sys.exit(0)
...
if "torus" == netTopo:
    ...
    # Sanity check, to be improved
    if boardShape == "" or globShape == "" or fatTreeShape == "":
        print("Error, please specify at least --boardShape --globShape --fatTreeShape")
        sys.exit(0)

    if usePlatNetConfig:
        topoInfo = HammingInfo(platNetConfig)
    else:
        if (fatTreeShape == ""):
            fatTreeShape = "1:1"
        if (routing_algo == ""):
            routing_algo = "min-adaptive"
        topoInfo = HammingInfo(boardShape, netHostsPerRtr, globShape, fatTreeShape, routing_algo)
...
#nicParams["numSendMachines"] = 4
#nicParams["numRecvNicUnits"] = 4
...
if (netTopo == "hx" or netTopo == "torus" or netTopo == "hyperx" or netTopo == "torus3d"):
    if netBW == "1600Gb/s" or netBW == "3200Gb/s":
        networkParams['nic_link_bw'] = "12800Gb/s"
        networkParams['xbar_bw'] = "12800Gb/s"
    else:
        networkParams['nic_link_bw'] = "12800Gb/s"
        networkParams['xbar_bw'] = "12800Gb/s"
else:
    # If we are running ResNet or GPT
    if ("ring" not in loadFile.lower() or True):
        networkParams['nic_link_bw'] = "1600Gb/s"
        networkParams['link_bw'] = "1600Gb/s"
        networkParams['xbar_bw'] = "3200Gb/s"
    else:
        networkParams['nic_link_bw'] = "400Gb/s"
        networkParams['link_bw'] = "400Gb/s"
        networkParams['xbar_bw'] = "800Gb/s"
    
print("link_bw " + str(networkParams['link_bw']))
print("xbar_bw " + str(networkParams['xbar_bw']))
print("nic_link_bw " + str(networkParams['nic_link_bw']))

nicParams["packetSize"] =    networkParams['packetSize']
#nicParams["link_bw"] = networkParams['link_bw']
nicParams["link_bw"] = networkParams['nic_link_bw']
sst.merlin._params["link_lat"] = networkParams['link_lat']
sst.merlin._params["link_bw"] = networkParams['link_bw']
sst.merlin._params["nic_link_bw"] = networkParams['nic_link_bw']
...
baseNicParams = {
    ...
    "link_bw" : networkParams['nic_link_bw'],

...
#ifndef _H_EMBER_BRUCKCOLL_MOTIF
#define _H_EMBER_BRUCKCOLL_MOTIF
...
class EmberBruckCollGenerator : public EmberMessagePassingGenerator {
...
private:
    ...
    // BruckCollectiveEngine engine. Used as building blocks to implement multidimensional collectives.
    class BruckCollectiveEngine {

    public:
        BruckCollectiveEngine(EmberBruckCollGenerator &gen, CollType coll_type, uint* dimensions, uint dimensions_num,
    ...
    // Abstract class that defines a generic bruck collective implementation running multiple collectives concurrently
    class BruckCollectiveRunner 
    {
    public:
        BruckCollectiveRunner(EmberBruckCollGenerator &gen, int num_allreduces, uint32_t count, uint32_t rank, uint32_t comm_size, 
    ...
    protected:
        std::vector<BruckCollectiveEngine> m_allreduces;
        std::vector<MessageRequest> m_active_recv_handles;
        std::vector<BruckCollectiveEngine*> m_active_allreduce_ptrs;
...
public:
    // BruckCollective
    // This is a bruck collective on a d-dimensional torus.
    // It is 'Single' in the sense that there is a single collective running on m_count elements.
    class BruckCollective: public BruckCollectiveRunner
    {
    public:
        BruckCollective(EmberBruckCollGenerator &gen, uint dimensions_num, uint ports,
                        uint32_t count, uint32_t rank, uint32_t comm_size, 
                        Communicator comm, double aggregation_cost_ns = 0, bool nb = false,
                        bool sync = true, CollType coll_type = BRUCK_ALLREDUCE, float* data = NULL,
...
public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
        EmberBruckCollGenerator,
        "ember",
        "EmberBruckMotif",
        SST_ELI_ELEMENT_VERSION(1,0,0),
        "EmberBruckMotif parent motif -- this is meant to be a superclass of EmberBruckMotif motifs",
        SST::Ember::EmberGenerator
    )

public:
	EmberBruckCollGenerator(SST::ComponentId_t, Params& params);
	EmberBruckCollGenerator(SST::ComponentId_t, Params& params, std::string name);

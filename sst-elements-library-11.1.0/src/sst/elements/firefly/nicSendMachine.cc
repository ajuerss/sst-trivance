...
void Nic::SendMachine::streamInit( SendEntryBase* entry )
{
    ...
    //printf("[%d] %" PRIu64 " sendmachine (%d) stream init (dest: %d, size: %d)\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), m_id, entry->dest(), entry->totalBytes());
...
void Nic::SendMachine::getPayload( SendEntryBase* entry, FireflyNetworkEvent* ev )
{
    ...
    if ( ! m_inQ->isFull() ) {
        ...
        if ( entry->isDone() ) {
            //printf("[%d] %" PRIu64 " sendmachine (%d) entry is done (dest: %d)\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), m_id, entry->dest());
            ev->setTail();
            m_inQ->enque( m_unit, pid, vec, ev, entry->vn(), entry->dest(), std::bind( &Nic::SendMachine::streamFini, this, entry ) );
        } else {
            //printf("[%d] dest: %d; m_packetSizeInBytes: %d;\n", m_nic.getNodeId(), entry->dest(), m_packetSizeInBytes);
            m_inQ->enque( m_unit, pid, vec, ev, entry->vn(), entry->dest() );
            m_nic.schedCallback( std::bind( &Nic::SendMachine::getPayload, this, entry, new FireflyNetworkEvent(m_pktOverhead) ), 1);
        }

    } else {
        //printf("[%d] sendmachine (%p) going to sleep (dest: %d)\n", m_nic.getNodeId(), m_inQ, entry->dest());
...
void Nic::SendMachine::streamFini( SendEntryBase* entry )
{
    ...
    if ( entry->shouldDelete() ) {
        //printf("[%d] %" PRIu64 " sendmachine %d deleting entry (dest: %d)\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), m_id, entry->dest());
            ...
            FireflyNetworkEvent* ev, int vn, int dest, Callback callback )
{
    ...
    //printf("[%d] %" PRIu64" sendmachine (inQ: %p) preparing pkt for %d\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), this, dest);
...
void Nic::SendMachine::InQ::ready( FireflyNetworkEvent* ev, int vn, int dest, Callback callback, uint64_t pktNum )
{
    ...
    //printf("[%d] %" PRIu64" sendmachine (inQ: %p) pkt for %d is ready (DMA completed)\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), this, dest);
...
void Nic::SendMachine::OutQ::enque( FireflyNetworkEvent* ev, int vn, int dest, Callback callback )
{
    ...
    //printf("[%d] %" PRIu64 " sendmachine injecting pkt to %d\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), dest);
	...
	m_nic.notifyHavePkt( m_id, px, vn );
}

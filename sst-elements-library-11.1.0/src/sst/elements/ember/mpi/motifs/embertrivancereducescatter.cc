...
#include "embertrivancereducescatter.h"

using namespace SST::Ember;

EmberTrivanceReduceScatterGenerator::EmberTrivanceReduceScatterGenerator(SST::ComponentId_t id, Params &params) 
: EmberTrivanceCollGenerator(id, params, "TrivanceReduceScatter")
{
    ...
    m_reducescatter = new TrivanceCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, aggregation_cost_ns, 
                                          !blocking, sync, TRIVANCE_REDUCE_SCATTER, NULL, dimensions_sizes);
}

EmberTrivanceReduceScatterGenerator::~EmberTrivanceReduceScatterGenerator()
{
    m_reducescatter->printStats();
    delete m_reducescatter;
}

bool EmberTrivanceReduceScatterGenerator::generate(std::queue<EmberEvent *> &evQ)
{

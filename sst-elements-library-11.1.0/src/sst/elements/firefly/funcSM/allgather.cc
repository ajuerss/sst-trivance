...
bool AllgatherFuncSM::setup( Retval& retval )
{
    ...
    switch ( m_setupState.state ) {
      ...
      case SetupState::PostStartMsgRecv:
        ...
        proto()->irecv( addr, 0, src, genTag(), m_event->group, &m_recvReq );
      ...
      case SetupState::PostStageRecv:
        ...
        proto()->irecvv( ioVec, src, genTag() + m_setupState.stage + 1, m_event->group, &m_recvReqV[m_setupState.stage] );
      ...
      case SetupState::SendStartMsg:
		...
		addr.setSimVAddr( 1 );
		addr.setBacking( NULL );
        //printf("allgather; sending to: %d; m_dest[0]: %d\n", m_info->getGroup(m_event->group)->getMapping( m_dest[0]), m_dest[0]);
        //proto()->send( addr, 0, m_info->getGroup(m_event->group)->getMapping( m_dest[0] ), genTag(), m_smallCollectiveVN );
        proto()->send( addr, 0, m_dest[0], genTag(), m_event->group, m_smallCollectiveVN );
        //proto()->send( addr, 0, m_dest[0], genTag(), m_smallCollectiveVN );
    }
    return true;
}
...
void AllgatherFuncSM::handleEnterEvent( Retval& retval )
{
    ...
    case SendData:
...

        {
            ...
            //proto()->sendv( ioVec, m_info->getGroup(m_event->group)->getMapping(m_dest[m_currentStage]), genTag() + m_currentStage + 1, vn );
            proto()->sendv( ioVec, m_dest[m_currentStage], genTag() + m_currentStage + 1, m_event->group, vn );
        }

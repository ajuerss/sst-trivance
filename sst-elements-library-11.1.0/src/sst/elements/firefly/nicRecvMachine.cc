...
void Nic::RecvMachine::processPkt( FireflyNetworkEvent* ev ) {
    ...
    if (ev->bufSize() > 50) {
        m_nic.received_bytes += ev->bufSize();
        m_nic.received_packets++;
    }
    if (ev->bufSize() > 50 && (m_nic.received_packets % 10000 == 0)) {
        time_t mytime = time(NULL);
        char * time_str = ctime(&mytime);
        time_str[strlen(time_str)-1] = '\0';
        printf("NIC %d is at packet %" PRIu64 " - Time %s\n",m_nic.m_myNodeId, m_nic.received_packets, time_str); 
...
void Nic::RecvMachine::processStdPkt( FireflyNetworkEvent* ev ) {
    ...
    if ( ev->isHdr() ) {
        //printf("[%d] %" PRIu64" new stream %p %s node=%d pid=%d for pid=%d\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), stream, ev->isTail()? "single packet stream":"", ev->getSrcNode(),ev->getSrcPid(), pid );
        ...
        if ( ev->isTail() ) {
            //printf("[%d] %" PRIu64 " tail pkt, clear streamMap %p node=%d pid=%d for pid=%d\n", m_nic.getNodeId(), m_nic.getCurrentSimTimeNano(), stream, ev->getSrcNode(),ev->getSrcPid(), pid);

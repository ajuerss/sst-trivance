...
#include "nicSendMachineRRArbiter.h"
...
struct X {
	...
	std::vector< std::vector< std::priority_queue< PriorityX*,std::vector<PriorityX*>, Compare > > > m_sendPQ;
    std::vector<SendMachineRRArbiter*> m_sm_arb;
    std::vector<SendMachine*>   m_sendMachineV;
    std::queue<SendMachine*>    m_sendMachineQ;
    std::unordered_map<int, SendMachine*> m_sendMachineMap;
    ...
    void notifyHavePkt( int sm_id, PriorityX* px, int vn ) {
        m_dbg.debug(CALL_INFO,3,NIC_DBG_SEND_MACHINE,"vn=%d p1=%" PRIu64 " p2=%d\n",vn,px->p1(),px->p2());
        m_sendPQ[vn][sm_id].push( px );
        m_sm_arb[vn]->signalAvailability(sm_id);

        if ( 1 == m_sm_arb[vn]->getAvailabilityCount() ) {
            feedTheNetwork( vn );
        }
    }

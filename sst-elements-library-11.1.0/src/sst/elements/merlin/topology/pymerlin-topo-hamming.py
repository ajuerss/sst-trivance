...
class topoHamming(Topology):
    ...
    def _shape_callback(self,variable_name,value):
        ...
        if len(self._dim_size) != len(self._dim_width):
            print("topoHamming:  Incompatible number of dimensions set for shape (%s) and width (%s)."%(shape,width))
            exit(1)
    ...
    def getNumNodes(self):
        if not self._dim_size or not self.local_ports:
            print("topoHamming: calling getNumNodes before shape, width and local_ports were set.")
            exit(1)
    ...
    def getLocalDims(self, location):
        return [x % self.dims[0] for x in location]  #Only works for squares
    ...
    def build(self, endpoint):
        ...
        for i in range(num_routers):
            ...
            topology = rtr.setSubComponent(self.router.getTopologySlotName(),"merlin.hamming")

...
class HammingInfo(TopoInfo):
    ...
    def __init__( self, param1, param2, param3, param4, param5=None ):
        self.params = {}
        if param2:
            self.initV1( param1, param2, param3, param4, param5)
        else:
            self.initV2( param1 )

    def initV1( self, shape, local_ports, global_shape, fat_tree_shape, algo ):
        width = 1
        self.params["num_dims"] = self.calcNumDim(shape)
        self.params["hamming:shape"] = shape
        self.params["hamming:board_shape"] = shape
        self.params["hamming:global_shape"] = global_shape
        self.params["hamming:fat_tree_shape"] = fat_tree_shape
        self.params["hamming:width"] = self.calcWidth(shape,width)
        self.params["hamming:local_ports"] = local_ports
        ...
        print("Global Shape is " + str(self.params["hamming:global_shape"]))
        print("Fat Tree Shape is " + str(self.params["hamming:fat_tree_shape"]))
        print("width is " + str(self.calcWidth(shape,width)))
        print("local_ports is " + str(local_ports))
        print("algorithm is " + str(algo))
        print("numNodes is " + str(self.calcNumNodes( shape, global_shape ) * local_ports))

    def initV2( self, params ):
        local_ports = params["local_ports"]
        shape = params['shape']
        self.params["hamming:shape"] = shape
        self.params["hamming:width"] = params['width']
        self.params["num_dims"] = params['num_dims']
        self.params["hamming:local_ports"] = local_ports
        self.params["hamming:algorithm"] = params['algorithm']
        self.numNodes = self.calcNumNodes( shape ) * int(local_ports)
    ...
    def calcNumNodes(self,shape, global_shape):
        tmp = shape.split( 'x' )
        glob_tmp = global_shape.split( 'x' )
        num = 1
        for d in tmp:
            num = num * int(d)
        for d in glob_tmp:

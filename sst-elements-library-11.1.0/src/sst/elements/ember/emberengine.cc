...
void EmberEngine::finish() {
    ...
    uint64_t tot_motif_time = m_motif_end_time - m_motif_start_time;
    uint64_t bytes = getLib("mpi")->getTotalSentBytes();
    printf("STATS %d %d EMBER %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "\n", m_jobId, m_os->getRank(), m_motif_start_time, m_motif_end_time, tot_motif_time, bytes);
...
void EmberEngine::setup() {
    ...
    m_generator->enQ_getTime(evQueue, &m_motif_start_time);
    ...
    while ( evQueue.empty() ) {
        ...
        if (  evQueue.empty() ) {
            ...
            if ( m_generator->primary() ) {
                m_generator->enQ_getTime(evQueue, &m_motif_end_time);

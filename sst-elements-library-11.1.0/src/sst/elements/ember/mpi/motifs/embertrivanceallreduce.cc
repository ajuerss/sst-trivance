...
#include "embertrivanceallreduce.h"

using namespace SST::Ember;

EmberTrivanceAllreduceGenerator::EmberTrivanceAllreduceGenerator(SST::ComponentId_t id, Params &params) 
: EmberTrivanceCollGenerator(id, params, "TrivanceAllreduce")
{
    ...
    m_allreduce = new TrivanceCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, aggregation_cost_ns, 
                                      !blocking, sync, TRIVANCE_ALLREDUCE, m_data, dimensions_sizes, latency_optimal);
}

EmberTrivanceAllreduceGenerator::~EmberTrivanceAllreduceGenerator()
{
    m_allreduce->printStats();
    delete m_allreduce;
}

bool EmberTrivanceAllreduceGenerator::generate(std::queue<EmberEvent *> &evQ)
{

...
{
    for (int i = 0; i < 10; i++) {
        //printf("Switch %d - Port %d used %d times (%d)\n",router_id, i, port_used[i], local_port_start);
    }
    delete [] id_loc;
    delete [] dim_size;
    delete [] dim_width;
    ...
    if ( dest_router == router_id ) {
        ...
        for ( int dim = tt_ev->routing_dim ; dim < dimensions ; dim++ ) {
            if ( tt_ev->dest_loc[dim] != id_loc[dim] ) {
                ...
                if(dist_pos == dist_neg){ // If they are equidistant, pick the best direction
                    int pos_port = port_start[dim][0] + (dist_pos % dim_width[dim]);
                    int neg_port = port_start[dim][1] + (dist_neg % dim_width[dim]);
                    int next_vc = (id_loc[dim] == 0 && port < local_port_start) ? vc^1 : vc;
                    int credits_pos = output_credits[pos_port * num_vcs + next_vc];
                    int credits_neg = output_credits[neg_port * num_vcs + next_vc];
                    go_pos = (credits_pos > credits_neg);
                }
...
{
    printf("Torus routing init data\n");
...
void
topo_torus::setOutputBufferCreditArray(int const* array, int vcs)
{
...
void
topo_torus::setOutputQueueLengthsArray(int const* array, int vcs)
{

...
#ifndef _H_EMBER_SWINGCOLL_MOTIF
#define _H_EMBER_SWINGCOLL_MOTIF
...
class EmberSwingCollGenerator : public EmberMessagePassingGenerator {
...
private:
    ...
    // SwingCollectiveEngine engine. Used as building blocks to implement multidimensional collectives.
    class SwingCollectiveEngine {

    public:
        SwingCollectiveEngine(EmberSwingCollGenerator &gen, CollType coll_type, uint* dimensions, uint dimensions_num,
    ...
    // Abstract class that defines a generic swing collective implementation running multiple collectives concurrently
    class SwingCollectiveRunner 
    {
    public:
        SwingCollectiveRunner(EmberSwingCollGenerator &gen, int num_allreduces, uint32_t count, uint32_t rank, uint32_t comm_size, 
    ...
    protected:
        std::vector<SwingCollectiveEngine> m_allreduces;
        std::vector<MessageRequest> m_active_recv_handles;
        std::vector<SwingCollectiveEngine*> m_active_allreduce_ptrs;
...
public:
    // SwingCollective
    // This is a swing collective on a d-dimensional torus.
    // It is 'Single' in the sense that there is a single collective running on m_count elements.
    class SwingCollective: public SwingCollectiveRunner
    {
    public:
        SwingCollective(EmberSwingCollGenerator &gen, uint dimensions_num, uint ports,
                        uint32_t count, uint32_t rank, uint32_t comm_size, 
                        Communicator comm, double aggregation_cost_ns = 0, bool nb = false,
                        bool sync = true, CollType coll_type = SWING_ALLREDUCE, float* data = NULL,
    ...
    {
    public:
        EmberImprovedAlltoall(EmberSwingCollGenerator &gen, uint32_t sendcount, uint32_t rank, uint32_t comm_size, Communicator comm);
...
public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
        EmberSwingCollGenerator,
        "ember",
        "EmberSwingMotif",
        SST_ELI_ELEMENT_VERSION(1,0,0),
        "EmberSwingMotif parent motif -- this is meant to be a superclass of EmberSwingMotif motifs",
        SST::Ember::EmberGenerator
    )

public:
	EmberSwingCollGenerator(SST::ComponentId_t, Params& params);
	EmberSwingCollGenerator(SST::ComponentId_t, Params& params, std::string name);

    ...
    m_getRespSmallVN(0)
{
    ...
    for (int i = 0; i < m_numVN; i++) m_sendPQ[i].resize(numSendMachines);

    m_sm_arb.resize(m_numVN);
    for (int i = 0; i < m_numVN; i++) m_sm_arb[i] = new SendMachineRRArbiter(numSendMachines);
    ...
    for ( int i = 0; i < m_numVN; i++ ) {
        m_recvMachine.push_back( new RecvMachine( *this, i, m_vNicV.size(), m_myNodeId,
                ...
                params.find<int>( "maxActiveRecvStreams", 32768 ),
...
Nic::~Nic()
{
    printf("Nic %d received %d bytes\n", m_myNodeId, received_bytes); fflush(stdout);
	delete m_shmem;
	delete m_unitPool;
 	delete m_linkSendWidget;
	delete m_linkRecvWidget;
    for (int i = 0; i < m_numVN; i++) delete m_sm_arb[i];
...
void Nic::init( unsigned int phase )
{
    ...
    if ( m_linkBytesPerSec == 0 && m_linkControl->isNetworkInitialized() ) {
        //printf("NIC->switch bw: %s\n", m_linkControl->getLinkBW().toString().c_str());
...
void Nic::dmaSend( NicCmdEvent *e, int vNicNum )
{
    ...
    //printf("Posting Send %d dma\n", m_myNodeId); fflush(stdout);
...
void Nic::qSendEntry( SendEntryBase* entry ) {
    ...
    if ( entry->isCtrl() || entry->isAck() ) {
        //printf("[%d] CTRL Looking for existing sendmachine for dest: %d\n", getNodeId(), entry->dest());
        entry->setTxDelay(0);
        ...
        //printf("[%d] Looking for existing sendmachine for dest: %d\n", getNodeId(), entry->dest());
        if (m_sendMachineMap.find(entry->dest()) != m_sendMachineMap.end()) {
            assert(m_sendMachineMap[entry->dest()] != NULL);
            //printf("[%d] found active sendmachine (%d) for dest: %d -> pushing\n", getNodeId(), m_sendMachineMap[entry->dest()]->getId(), entry->dest());
            m_sendMachineMap[entry->dest()]->m_qsend.push(entry);
        } else if ( ! m_sendMachineQ.empty() ) {
            assert( m_sendEntryQ.empty() );
            assert(m_sendMachineQ.front() != NULL);
            //printf("[%d] allocating new sendmachine (id: %d) for dest: %d\n", getNodeId(), m_sendMachineQ.front()->getId(), entry->dest());
            m_sendMachineQ.front()->run( entry );
            m_sendMachineMap[entry->dest()] = m_sendMachineQ.front();
            m_sendMachineQ.pop();
        } else {
...
void Nic::notifySendDone( SendMachine* mach, SendEntryBase* entry  ) {
    ...
    if (!mach->m_qsend.empty()) {
        //printf("[%d] found queued send (dest: %d) for sendmachine %d\n", getNodeId(), entry->dest(), mach->getId());
        m_sendMachineMap[mach->m_qsend.front()->dest()] = mach;
        mach->run(mach->m_qsend.front());
        mach->m_qsend.pop();
        return;
    } else if(mach != m_sendMachineV[m_sendMachineV.size()-1]) {
        //the last sendmachine in m_sendMachineV is the one used for CTRL messages
        //and that does not participate in the per-dest serialization stuff
        //printf("[%d] freeing up sendmachine (send: %d)\n", getNodeId(), entry->dest());
        m_sendMachineMap.erase(entry->dest());
    }

    if ( m_sendEntryQ.empty() ) {
        m_sendMachineQ.push(mach);
    } else {
        // now we need to consider the case where a send is waiting and while that is waiting, 
        // another send, towards the same destination, gets assigned to a sendmachine.
        // Hence, here we need to check if this sendmachine can actually take that send (it cannot
        // if there is another sendmachine serving sends towards the same destination)
        bool found_one = false;
        while (!m_sendEntryQ.empty() && !found_one)
        {
            SendEntryBase *entry = m_sendEntryQ.front().second;
            if (m_sendMachineMap.find(entry->dest()) != m_sendMachineMap.end()) {
                //printf("[%d] notifySendDone found active sendmachine for dest: %d -> pushing\n", getNodeId(), entry->dest());
                m_sendMachineMap[entry->dest()]->m_qsend.push(entry);
            }
            else
            {
                //printf("[%d] notifySendDone found unmapped send (dest: %d) -> taking it\n", getNodeId(), entry->dest());
                assert(mach!=NULL);
                m_sendMachineMap[entry->dest()] = mach;
                mach->run(entry);
                found_one = true;
            }
            m_sendEntryQ.pop();
        ...
        if (!found_one)
        {
            //printf("[%d] notifySendDone there were pending sends but all already mapped to other machs -> idle\n", getNodeId());
            m_sendMachineQ.push(mach);
        }
    }
}
...
void Nic::feedTheNetwork( int vn )
{
    ...
    while (!m_sm_arb[vn]->isEmpty())
    {
        //m_sm_arb[vn]->printStatus(getNodeId());
        int sm_idx = m_sm_arb[vn]->getNextIdx();
        
        auto &pq = m_sendPQ[vn][sm_idx];
        assert(!pq.empty());
		PriorityX* entry = pq.top();
		X& x = *entry->data();
        //printf("[%d] %" PRIu64 " feedTheNetwork: vn: %d; sm_idx: %d; dest: %d; isCtrl: %d; isHdr: %d; isTail: %d;\n", getNodeId(), getCurrentSimTimeNano(), vn, sm_idx, x.dest, (int) x.pkt->isCtrl(), (int) x.pkt->isHdr(), (int) x.pkt->isTail());
		bool ret = m_linkControl->spaceToSend( vn, x.pkt->calcPayloadSizeInBits() );
		if ( ! ret ) {
            //printf("[%d] feedTheNetwork: vn %d; sm_idx: %d NO SPACE\n", getNodeId(), vn, sm_idx);
            ...
            //printf("[%d] injection overhead: %" PRIu64 "; m_linkBytesPerSec: %lu\n", getNodeId(), latPS, m_linkBytesPerSec);
            //printf("[%d] sending pkt to %d (vn: %d)\n", getNodeId(), x.dest, vn);
			if ( curTime > m_predNetIdleTime ) {
            ...
            m_sm_arb[vn]->popNextIdx();
            // the following is not true for CTRL messages, which are sent out-of-band and are not involved
            // into the per-dest serialization stuff
            //bool belongs_to_this_mach = m_sendMachineMap[x.dest] == m_sendMachineV[sm_idx];
            //int pkt_left_for_this_mach = m_sm_arb[vn]->popNextIdx();
            //if (belongs_to_this_mach && pkt_left_for_this_mach == 0 && x.pkt->isTail()) 
            //{
            //    printf("[%d] freeing up sendmachine %d (dest: %d)\n", getNodeId(), sm_idx, x.dest);
            //    m_sendMachineMap.erase(x.dest);
            //}

...
#ifndef _H_EMBER_RECDOUBCOLL_MOTIF
#define _H_EMBER_RECDOUBCOLL_MOTIF
...
class EmberRecDoubCollGenerator : public EmberMessagePassingGenerator {
...
private:
    ...
    public:
        enum CollType {
            RECDOUB_ALLREDUCE = 0,
            RECDOUB_REDUCE_SCATTER,
            RECDOUB_ALLGATHER
        };

    // RecDoubCollectiveEngine engine. Used as building blocks to implement multidimensional collectives.
    class RecDoubCollectiveEngine {

    public:
        RecDoubCollectiveEngine(EmberRecDoubCollGenerator &gen, CollType coll_type, uint* dimensions, uint dimensions_num,
                        float *dst, uint32_t count, uint32_t vrank, uint32_t numproc, 
                        double aggregation_cost_ns, Communicator comm, bool validate, uint port, bool latency_optimal = false);
    ...
    private:
        ...
        void computeReducescatterBlocksBitmap(int sender, int step, uint8_t* blocks_bitmap);
        void computeAllgatherBlocksBitmap(int sender, int step, uint8_t* blocks_bitmap);
    ...
    private:
        EmberRecDoubCollGenerator &m_gen;
        float *m_dst;
        float *m_tmp;
        float *m_send_tmp;
        uint32_t m_p;
    ...
    // Abstract class that defines a generic recursive doubling collective implementation running multiple collectives concurrently
    class RecDoubCollectiveRunner 
    {
    public:
        RecDoubCollectiveRunner(EmberRecDoubCollGenerator &gen, int num_allreduces, uint32_t count, uint32_t rank, uint32_t comm_size, 
    ...
    protected:
        std::vector<RecDoubCollectiveEngine> m_allreduces;
        std::vector<MessageRequest> m_active_recv_handles;
        std::vector<RecDoubCollectiveEngine*> m_active_allreduce_ptrs;
...
public:
    // RecDoubCollective
    // This is a recursive doubling collective on a d-dimensional torus.
    // It is 'Single' in the sense that there is a single collective running on m_count elements.
    class RecDoubCollective: public RecDoubCollectiveRunner
    {
    public:
        RecDoubCollective(EmberRecDoubCollGenerator &gen, uint dimensions_num, uint ports,
                        uint32_t count, uint32_t rank, uint32_t comm_size, 
                        Communicator comm, double aggregation_cost_ns = 0, bool nb = false,
                        bool sync = true, CollType coll_type = RECDOUB_ALLREDUCE, float* data = NULL,
...
public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
        EmberRecDoubCollGenerator,
        "ember",
        "EmberRecDoubMotif",
        SST_ELI_ELEMENT_VERSION(1,0,0),
        "EmberRecDoubMotif parent motif -- this is meant to be a superclass of EmberRecDoubMotif motifs",
        SST::Ember::EmberGenerator
    )

public:
	EmberRecDoubCollGenerator(SST::ComponentId_t, Params& params);
	EmberRecDoubCollGenerator(SST::ComponentId_t, Params& params, std::string name);

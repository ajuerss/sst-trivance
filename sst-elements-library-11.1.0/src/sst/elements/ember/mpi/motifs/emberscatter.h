...
/*
#ifndef _H_EMBER_SCATTER_MOTIF
#define _H_EMBER_SCATTER_MOTIF
...
public:           
    ...
    int size; 
    int sleep; 
    EventEntry(int rank, int size, int sleep) { 
      this->rank = rank;
      this->size = size;
      this->sleep = sleep;
    }
...
public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
        {   "arg.messageSize",      "Sets the message size of the communications (in bytes)", "1"},
        {   "arg.iterations",       "Sets the number of iterations to perform",     "1"},
        {   "arg.waitToStart",       "Wait to start",     "0"},
    )
...
protected:
    ...
    uint32_t is_dragonfly;
    float load_factor = 0;
    int nextMRIndex = 0;
    MessageRequest* msgRequests;
    MessageRequest msgRequests2;
    MessageRequest msgRequests3;
	uint32_t iteration = 0;
    bool can_finish = false;
	uint32_t msgSize;
        bool started;
        int waitToStart;
    std::vector<EventEntry> sending_list;
    std::vector<EventEntry> receiving_list;

...
EmberSwingAllreduceGenerator::EmberSwingAllreduceGenerator(SST::ComponentId_t id, Params &params) 
: EmberSwingCollGenerator(id, params, "SwingAllreduce")
{
    ...
    m_allreduce = new SwingCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, aggregation_cost_ns, 
                                      !blocking, sync, SWING_ALLREDUCE, m_data, dimensions_sizes, latency_optimal);
}

EmberSwingAllreduceGenerator::~EmberSwingAllreduceGenerator()
{
    m_allreduce->printStats();
    delete m_allreduce;
}

bool EmberSwingAllreduceGenerator::generate(std::queue<EmberEvent *> &evQ)
{

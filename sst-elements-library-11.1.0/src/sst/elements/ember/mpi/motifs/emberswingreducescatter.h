#ifndef _H_EMBER_SWINGREDUCESCATTER_MOTIF
#define _H_EMBER_SWINGREDUCESCATTER_MOTIF
...
class EmberSwingReduceScatterGenerator : public EmberSwingCollGenerator {

public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
        EmberSwingReduceScatterGenerator,
        "ember",
        "SwingReduceScatterMotif",
        SST_ELI_ELEMENT_VERSION(1,0,0),
        "Performs a SwingReduceScatter operation with type set to FLOAT and operation SUM",
...
public:
	EmberSwingReduceScatterGenerator(SST::ComponentId_t, Params& params);
    ~EmberSwingReduceScatterGenerator();
    bool generate(std::queue<EmberEvent*>& evQ);

private:
    SwingCollective* m_reducescatter;
};

...
#include "emberbruckallgather.h"

using namespace SST::Ember;

EmberBruckAllgatherGenerator::EmberBruckAllgatherGenerator(SST::ComponentId_t id, Params &params) 
: EmberBruckCollGenerator(id, params, "BruckAllgather")
{
    ...
    m_allgather = new BruckCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, 0, 
                                      !blocking, sync, BRUCK_ALLGATHER, NULL, dimensions_sizes);
}

EmberBruckAllgatherGenerator::~EmberBruckAllgatherGenerator()
{
    m_allgather->printStats();
    delete m_allgather;
}

bool EmberBruckAllgatherGenerator::generate(std::queue<EmberEvent *> &evQ)
{

...
#include "emberbruckreducescatter.h"

using namespace SST::Ember;

EmberBruckReduceScatterGenerator::EmberBruckReduceScatterGenerator(SST::ComponentId_t id, Params &params) 
: EmberBruckCollGenerator(id, params, "BruckReduceScatter")
{
    ...
    m_reducescatter = new BruckCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, aggregation_cost_ns, 
                                          !blocking, sync, BRUCK_REDUCE_SCATTER, NULL, dimensions_sizes);
}

EmberBruckReduceScatterGenerator::~EmberBruckReduceScatterGenerator()
{
    m_reducescatter->printStats();
    delete m_reducescatter;
}

bool EmberBruckReduceScatterGenerator::generate(std::queue<EmberEvent *> &evQ)
{

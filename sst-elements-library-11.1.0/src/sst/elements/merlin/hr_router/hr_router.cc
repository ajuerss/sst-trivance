...
hr_router::~hr_router()
{
    ...
    for (int i = 0; i < pkts_per_port.size(); i++) {
        //printf("%d-%d packets for this port %d\n", id, i, pkts_per_port[i]);
    }
    ...
    output(Simulation::getSimulation()->getSimulationOutput())
{
    ...
    // Stats per Port
    pkts_per_port = std::vector<int>(num_ports);
    ...
    for ( int i = 0; i < num_ports; i++ ) {
        ...
        if (topo->isHostPort(i)) {
            pc_params.insert("link_bw", getLogicalGroupParam(params,topo,i,"nic_link_bw") );
        } else {
            pc_params.insert("link_bw", getLogicalGroupParam(params,topo,i,"link_bw") );
        }
        

        if (topo->is_hx && topo->is_board_switch) {
            pc_params.insert("link_lat", "1ns");
            //pc_params.insert("output_latency","100ns");
            //pc_params.insert("input_latency","100ns");
            //printf("Changed Router specs\n");
        } else {
            //printf("In/out latency is %s\n", getLogicalGroupParam(params,topo,i,"input_latency","0ns").c_str());
...
hr_router::clock_handler(Cycle_t cycle)
{
    ...
    for ( int i = 0; i < num_ports; i++ ) {
        ...
        if ( progress_vcs[i] > -1 ) {
            ...
            if (ev->getEncapsulatedEvent()->getSizeInBits() / 8 > 45) {
                pkts_per_port[ev->getNextPort()]++;
            }
            
            // printf("Sending from %d to %d, size %d\n", ev->getSrc(), ev->getDest(), ev->getEncapsulatedEvent()->getSizeInBits() / 8); fflush(stdout);

...
#include "emberbruckallreduce.h"

using namespace SST::Ember;

EmberBruckAllreduceGenerator::EmberBruckAllreduceGenerator(SST::ComponentId_t id, Params &params) 
: EmberBruckCollGenerator(id, params, "BruckAllreduce")
{
    ...
    m_allreduce = new BruckCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, aggregation_cost_ns, 
                                      !blocking, sync, BRUCK_ALLREDUCE, m_data, dimensions_sizes, latency_optimal);
}

EmberBruckAllreduceGenerator::~EmberBruckAllreduceGenerator()
{
    m_allreduce->printStats();
    delete m_allreduce;
}

bool EmberBruckAllreduceGenerator::generate(std::queue<EmberEvent *> &evQ)
{

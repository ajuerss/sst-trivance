...
{
    //printf("C %d\n", xbar_in_credits[vc]); fflush(stdout);
    ...
    total_incast_flits(0)
{
    ...
    if ( port_link == NULL ) {
        connected = false;
        //printf("Rtr %d - Not connected, %s\n",rtr_id, link_port_name.c_str()); fflush(stdout);
        return;
    }
...
RtrInitEvent* PortControl::checkInitProtocol(Event* ev, RtrInitEvent::Commands command, uint32_t line, const char* file, const char* func)
{
    ...
    if ( nullptr == ev || static_cast<BaseRtrEvent*>(ev)->getType() != BaseRtrEvent::INITIALIZATION ) {
        good = false;
        //printf("Problem1"); fflush(stdout);
        if (ev == nullptr) {
            //printf("Null\n"); fflush(stdout);
        }
    }
...
PortControl::handle_input_n2r(Event* ev)
{
	...
	case BaseRtrEvent::PACKET:
	{
        ...
        //printf("PKTOUT %" PRIu64 " RTR.%d %d %d %d\n", getCurrentSimTimeNano(), rtr_id, event->getTrustedSrc(), event->getDest(), event->getSizeInBits());
...
PortControl::handle_output(Event* ev) {
    ...
    if ( vc_to_send != -1 ) {
	    ...
	    if ( host_port ) {
            //printf("PKTIN %" PRIu64 " RTR.%d %d %d %d\n", getCurrentSimTimeNano(), rtr_id, send_event->getSrc(), send_event->getDest(), send_event->getEncapsulatedEvent()->getSizeInBits());

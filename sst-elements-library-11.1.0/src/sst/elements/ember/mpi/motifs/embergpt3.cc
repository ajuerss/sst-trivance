...
bool EmberGPT3Generator::setup_comms(std::queue<EmberEvent *> &evQ)
{
    ...
    case 8:
    {
        ...
        if (!all_reduce_type.compare("05D")) {
            ...
            float* m_data = NULL;
            uint* dimensions_sizes;
            int ports = dimensions*2;
            bool sync = true;
            m_mp_allreduce = new EmberRingAllreduceRev(*this, MP_ALLREDUCE_SIZE, m_mp_allreduce_group_rank, m_mp_allreduce_group_size, m_mp_allreduce_comm, m_aggregation_cost_ns, false, dimensions, dimensions_sizes, RING_ALLREDUCE, m_data);
            m_dp_allreduce = new EmberRingAllreduceRev(*this, DP_ALLREDUCE_SIZE, m_dp_allreduce_group_rank, m_dp_allreduce_group_size, m_dp_allreduce_comm, m_aggregation_cost_ns, false, dimensions, dimensions_sizes, RING_ALLREDUCE, m_data);
      ...
      m_iteration_state(INIT)
{
    ...
    if (all_reduce_type.compare("05D") && all_reduce_type.compare("Torus") && all_reduce_type.compare("Rings")) {

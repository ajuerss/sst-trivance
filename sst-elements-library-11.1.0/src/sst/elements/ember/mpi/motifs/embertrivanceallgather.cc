...
#include "embertrivanceallgather.h"

using namespace SST::Ember;

EmberTrivanceAllgatherGenerator::EmberTrivanceAllgatherGenerator(SST::ComponentId_t id, Params &params) 
: EmberTrivanceCollGenerator(id, params, "TrivanceAllgather")
{
    ...
    m_allgather = new TrivanceCollective(*this, dimensions, ports, recvcount, rank(), size(), GroupWorld, 0, 
                                      !blocking, sync, TRIVANCE_ALLGATHER, NULL, dimensions_sizes);
}

EmberTrivanceAllgatherGenerator::~EmberTrivanceAllgatherGenerator()
{
    m_allgather->printStats();
    delete m_allgather;
}

bool EmberTrivanceAllgatherGenerator::generate(std::queue<EmberEvent *> &evQ)
{
